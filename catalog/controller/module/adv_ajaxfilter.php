...
class ControllerModuleAdvajaxfilter extends Controller {
	...
	public function getProducts() {
		...
		foreach($par as $pa){
			if (strpos($pa, 'sort') !== false) {
				$new_sort = str_replace("sort=","",$pa);
			}
			
			if (strpos($pa, 'order') !== false) {
				$new_order = str_replace("order=","",$pa);
			}
			
			if (strpos($pa, 'limit') !== false) {
				$new_limit = str_replace("limit=","",$pa);
			}
