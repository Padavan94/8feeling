	...
	public function index() {
		...
		if ($category_info) {
			...
			if ($this->currency->getSymbolLeft($this->session->data['currency'])) {
				$code = $this->currency->getSymbolLeft($this->session->data['currency']);
				$data['right_code'] = false;
			} else {
				$code = $this->currency->getSymbolRight($this->session->data['currency']);
				$data['right_code'] = true;
			}
			
			$data['price_code'] = $code;
			$range = explode('-',$this->config->get('price_filter_range'));
			$data['price_range_min'] = $this->currency->convert($range[0],'XYZ',$this->session->data['currency']);
			$data['price_range_max'] = $this->currency->convert($range[1],'XYZ',$this->session->data['currency']);
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/price_filter.tpl')) {
				...
				return $this->load->view('module/price_filter.tpl', $data);
			}
